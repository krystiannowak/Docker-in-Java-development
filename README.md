 ...
 * one [JAX-RS GET resource](src/main/java/krystiannowak/helloworld/HelloWorldResource.java)
 
 * one `java.util.concurrent.atomic.AtomicLong` counter
 
 * one [simple config file](helloworld.yml)
...
* the plugin in use - [project's POM](pom.xml)
...
* example naive implementation in [HelloWorldHealthCheck](src/main/java/krystiannowak/helloworld/HelloWorldHealthCheck.java)
 ...
 * an example of testing PostgreSQL built-in procedures - [PostgresqlDBTest](src/test/java/krystiannowak/helloworld/db/PostgresqlDBTest.java)

* a generic test container can be used (or extended) to support e.g. MongoDB
 * an example of testing MongoDB server metadata - [MongoDBTest](src/test/java/krystiannowak/helloworld/db/MongoDBTest.java)
